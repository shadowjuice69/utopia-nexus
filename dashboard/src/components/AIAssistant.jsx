...
export default function AIAssistant() {
  ...
  async function ask() {
    ...
    try {
      ...
      const context = `
Kingdom ${MY_KD || "current"} current state:
Provinces: ${(provincesResult.data || []).map(p => `${p.name} (${p.race}, ${p.personality}, ${p.acres}a, ${p.nw}gc NW, Off:${p.off}, Def:${p.def})`).join("; ")}
  ...
  return (
    <div className="card">
      ...
      <div className="chat-input-row">
        <input className="chat-input" placeholder="Ask about targets, threats, waves, ops, or builds..." value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey} disabled={loading} />
        <button className="btn btn-gold" onClick={ask} disabled={loading}>Ask</button>
      </div>
    </div>
  );
}