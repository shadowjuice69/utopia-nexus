...
function normalize(row) {
  ...
  if (row.channel_type === "dragon") return { title: row.event_type || "Dragon event", detail: refs.length ? refs.join(" → ") : content || "Intel received", fallback: content, status: p.dragonName || "" };
  if (row.channel_type === "ritual") return { title: "Ritual", detail: refs.length ? refs.join(" → ") : content || "Intel received", fallback: content, status: p.success === false ? "FAILED" : p.success ? "SUCCESS" : "" };
  ...
  useEffect(() => {
    ...
    async function fetchRows() {
      try {
        ...
        if (!cancelled) {
          setRows([]);
          setError(e?.message || "Unable to load Intel 7.");
        }
      } finally {
    ...
    <div className="intel-panel">
      <div className="panel">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <h2 style={{ margin: 0 }}>📡 Seven-Channel Intel — {kd}</h2>
        ...
        {current.length === 0 ? <p style={{ color: "#64748b", textAlign: "center", padding: 28 }}>No new events recorded for {kd || "this kingdom"} on this channel yet.</p> : (
