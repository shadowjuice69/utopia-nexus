  ...
  const filtered = attacks.filter(a => {
    ...
    if (search) {
      ...
      return (
        ...
        a.attacker_kingdom?.toLowerCase().includes(q)
      );
    }
    return true;
  });
    ...
    <div className="intel-panel">
      <div className="panel">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: 8, marginBottom: 16 }}>
          {[
            { label: "ATTACKS (24H)", value: last24h.length, color: "#38bdf8" },
            { label: "OUTGOING", value: outgoing24.length, color: "#4ade80" },
            { label: "INCOMING", value: incoming24.length, color: "#ef4444" },
            { label: "ACRES (24H)", value: totalAcres > 0 ? `+${totalAcres.toLocaleString()}` : "0", color: "#facc15" },
            { label: "KILLS (24H)", value: totalKills.toLocaleString(), color: "#fb923c" },
          ].map(({ label, value, color }) => (
            <div key={label} style={{ background: `${color}1a`, border: `1px solid ${color}33`, borderRadius: 8, padding: "10px 14px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          <input
            placeholder="🔍 Search province, kingdom, type..."
            value={search}
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {filtered.map(a => {
              ...
              const inc = isIncoming(a);
              const emoji = ATTACK_EMOJI[a.event_type] || "⚔️";
              const borderColor = out ? "#4ade80" : inc ? "#ef4444" : "#475569";
              const data = a.data || {};
              ...
              return (
                <div key={a.id} style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                    <div>
                      ...
                      <span style={{ color: "#475569", fontSize: 12, marginLeft: 8 }}>
                        {out ? "outgoing" : inc ? "incoming" : "observed"}
                      </span>
                    </div>
                  ...
                  <div style={{ color: "#94a3b8", fontSize: 13, marginBottom: 8 }}>
                    <span style={{ color: out ? "#4ade80" : "#ef4444", fontWeight: 600 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    <StatPill label="Acres" value={acres} color="#facc15" />
                    <StatPill label="Kills" value={data.kills} color="#fb923c" />
                    <StatPill label="Imprisoned" value={data.imprisoned} color="#38bdf8" />
                    <StatPill label="Credits" value={data.credits} color="#8b5cf6" />
                    <StatPill label="Peasants" value={data.peasants} color="#4ade80" />
                    <StatPill label="Returns" value={data.return_days ? `${data.return_days}d` : null} color="#94a3b8" />
                    <StatPill label="vs Def" value={data.enemy_defense} color="#475569" />
                  </div>

                  {data.troops_lost && Object.keys(data.troops_lost).length > 0 && (
                    <div style={{ marginTop: 6, fontSize: 11, color: "#ef4444" }}>
                      Lost: {Object.entries(data.troops_lost).map(([k, v]) => `${v?.toLocaleString()} ${k}`).join(", ")}
                    </div>
                  )}
                </div>
              );
