...
export default function ThieveryCalculator() {
  ...
  const calculations = useMemo(() => {
    const selfTpa = modifiedTpa({
      acres: num(selfAcres, 1), thieves: num(selfThieves), thievesDensPct: num(thievesDens),
      thievesDensEffectivenessMultiplier: resolved.thievesDensEffectivenessMultiplier,
      crimeScienceMultiplier: num(crimeScienceMultiplier, 1), racialTpaMultiplier: resolved.racialTpaMultiplier,
      personalityTpaMultiplier: resolved.personalityTpaMultiplier, honorTpaMultiplier: num(honorTpa, 1),
      ritualTpaMultiplier: resolved.ritualTpaMultiplier, invisibilityMultiplier: 1, dragonTpaMultiplier: resolved.dragonTpaMultiplier,
    });
    ...
    const yieldResult = gains && maxPercent ? thieveryYield({
      thievesSent: thievesToSend, selfNetworth: num(selfNW, 1), targetNetworth: num(targetNW, 1), gainsPerThief: gains,
      resourcesLostFraction: op?.resourcesLostFraction ?? 0,
      modifiers: {
        racialMultiplier: resolved.racialTpaMultiplier, personalityMultiplier: resolved.personalityTpaMultiplier,
        scienceMultiplier: num(crimeScienceMultiplier, 1), guileMultiplier: guile ? 1.10 : 1, cunningMultiplier: 1,
        targetRacialMultiplier: resolved.sabotageDamageTakenMultiplier, targetPersonalityMultiplier: 1,
        targetIlluminateShadowsMultiplier: 1, targetWatchtowersReduction: watchTowersDamageReduction(num(targetWatchTowers)),
        targetShieldingReduction: num(shieldingReduction), stanceMultiplier: 1, warBonus: war ? 1 : 0,
      },
    }) : null;
  ...
  }, [selfAcres, selfThieves, selfNW, targetAcres, targetThieves, targetNW, targetResource, thievesDens, targetWatchTowers, crimeScienceMultiplier, shieldingReduction, honorTpa, operation, war, resolved, invisibility, guile]);
  ...
  return <div className="thievery-calculator">
    <div className="thievery-header">
      <div><h2>Thievery Calculator — Age 116</h2><p>Age 116 War Room mechanics. All displayed modifiers are data-driven and auditable.</p></div>
    ...
    <div className="thievery-grid">
      <section className="thievery-panel"><h3>Province</h3>
        ...
        <Field label="Thieves' Dens %" value={thievesDens} onChange={setThievesDens} step="0.1"/><Field label="Crime Science multiplier" value={crimeScienceMultiplier} onChange={setCrimeScienceMultiplier} step="0.01"/><Field label="Honor TPA multiplier" value={honorTpa} onChange={setHonorTpa} step="0.01"/>
      ...
      <section className="thievery-panel"><h3>Target & Operation</h3>
        ...
        <Field label="Target acres" value={targetAcres} onChange={setTargetAcres}/><Field label="Target thieves" value={targetThieves} onChange={setTargetThieves}/><Field label="Target networth" value={targetNW} onChange={setTargetNW}/>
        <Field label="Target resource amount" value={targetResource} onChange={setTargetResource}/><Field label="Target Watch Towers %" value={targetWatchTowers} onChange={setTargetWatchTowers} step="0.1"/><Field label="Target Shielding reduction" value={shieldingReduction} onChange={setShieldingReduction} step="0.01"/>
        <Field label="Current stealth %" value={stealth} onChange={setStealth} step="1"/>
    ...
    <section className="thievery-panel thievery-results"><h3>Calculation</h3><div className="thievery-result-grid">
      ...
      <div><span>Optimal thieves</span><strong>{calculations.thievesToSend == null ? 'Not fixed-yield' : calculations.thievesToSend.toLocaleString()}</strong></div>
      <div><span>Predicted yield</span><strong>{calculations.yieldResult == null ? 'Not fixed-yield' : calculations.yieldResult.value.toLocaleString(undefined, { maximumFractionDigits: 2 })}</strong></div>
    ...
    <section className="thievery-panel"><h3>Selected Age 116 modifiers</h3><div className="thievery-modifier-list">
      <span>Race: {race} ✓</span><span>Personality: {personality} ✓</span><span>Ritual: {ritual} ✓</span><span>Dragon: {dragon} ✓</span>
      <span>TD effectiveness: {resolved.thievesDensEffectivenessMultiplier.toFixed(2)}×</span><span>Sabotage damage: {resolved.sabotageDamageMultiplier.toFixed(3)}×</span><span>Sabotage success: {resolved.sabotageSuccessChanceMultiplier.toFixed(3)}×</span>
    </div></section>
