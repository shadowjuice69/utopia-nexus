...
function hasValidLogin(){const loggedAt=Number(localStorage.getItem("nexus_login_at")||0);const savedProvince=localStorage.getItem("nexus_province")||sessionStorage.getItem("nexus_province")||"";const savedUserId=localStorage.getItem("nexus_user_id")||"";return localStorage.getItem("nexus_auth")==="true"&&!!savedProvince&&!!savedUserId&&loggedAt>0&&Date.now()-loggedAt<LOGIN_TTL_MS;}
...
function clearLoginSession(){sessionStorage.removeItem("nexus_auth");sessionStorage.removeItem("nexus_province");localStorage.removeItem("nexus_auth");localStorage.removeItem("nexus_province");localStorage.removeItem("nexus_user_id");localStorage.removeItem("nexus_login_at");}
export default function App(){
 ...
 useEffect(()=>{let cancelled=false;async function restoreAuthorization(){if(!hasValidLogin()){clearLoginSession();if(!cancelled)setAuthReady(true);return;}restoreLoginSession();const registration=await getDashboardRegistration();const storedUserId=localStorage.getItem("nexus_user_id")||"";const allowed=registration.registered||registration.owner;const sameUser=!!registration.user&&registration.user.id===storedUserId;if(!cancelled){if(allowed&&sameUser)setAuthed(true);else{clearLoginSession();setAuthed(false);}setAuthReady(true);}}restoreAuthorization().catch(()=>{if(!cancelled){clearLoginSession();setAuthed(false);setAuthReady(true);}});return()=>{cancelled=true;};},[]);
 ...
 if(!authed)return <Login onAuth={result=>{sessionStorage.setItem("nexus_auth","true");localStorage.setItem("nexus_auth","true");localStorage.setItem("nexus_login_at",String(Date.now()));if(result?.user?.id)localStorage.setItem("nexus_user_id",result.user.id);if(result?.province?.name){sessionStorage.setItem("nexus_province",result.province.name);localStorage.setItem("nexus_province",result.province.name);}setAuthed(true);}}/>;
