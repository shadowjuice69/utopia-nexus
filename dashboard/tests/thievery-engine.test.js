...
test('Age 116 WT catch chance is 2.2% per percent WT', () => { close(watchTowersCatchChance(10), 0.22); assert.equal(watchTowersCatchChance(100), 1); });
test('WT damage reduction is 2.4% per WT capped at 62.5%', () => { close(watchTowersDamageReduction(10), 0.24); assert.equal(watchTowersDamageReduction(100), 0.625); });
...
test('published yield equation applies modifiers', () => { const result = thieveryYield({ thievesSent: 1000, selfNetworth: 100000, targetNetworth: 80000, gainsPerThief: 0.1, resourcesLostFraction: 0.5, modifiers: { racialMultiplier: 1.2, targetWatchtowersReduction: 0.1 } }); close(result.value, 43.2); });
