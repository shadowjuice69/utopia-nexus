...
function parseMessage(type, content) {
  ...
  if (type === 'attacks') {
    ...
    if (attack) {
      data.attacker_name = attack[1].trim();
      data.attacker_kd = attack[2];
      data.loot_amount = Number(attack[3].replace(/,/g, ''));
      data.loot_resource = attack[4].trim();
      data.target_name = attack[5].trim();
      data.target_kd = attack[6];
      data.event = 'attack';
    } else {
      data.event = 'attack';
      data.amount = numberAfter(text, /(?:looted|captured|gained)\s+([\d,]+)/i);
    }
  } else if (type === 'ops') {
    data.event = 'thievery';
    const op = text.match(/^(.*?)\s+\((\d+:\d+)\).*?(?:on|from|against)\s+(.*?)\s+\((\d+:\d+)\)/i);
    if (op) {
      data.attacker_name = op[1].trim();
      data.attacker_kd = op[2];
      data.target_name = op[3].trim();
      data.target_kd = op[4];
    }
    ...
    data.event = 'aid';
    data.amount = numberAfter(text, /([\d,]+)\s+(?:gold|food|runes|soldiers|units)/i);
  }
...
async function save(message, type, parsed) {
  ...
  if (!client) {
    logger.error('[INTEL7] Supabase is not configured');
    return false;
  }
  ...
  if (messageError) {
    logger.error(`[INTEL7] message save failed: ${messageError.message}`);
    return false;
  }
  ...
  if (eventError) {
    logger.error(`[INTEL7] event save failed: ${eventError.message}`);
    return false;
  }
...
function initialize(client) {
  ...
  client.on('messageCreate', async message => {
    ...
    try {
      ...
      logger.error(`[INTEL7 ERROR] type=${type} message=${message.id} ${error.stack || error.message}`);
    }
  });
