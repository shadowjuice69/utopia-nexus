...
async function loadCapture(sb, flow) {
  ...
  const { data, error } = await sb
    .from('spartan_capture_vault')
    .select('id,capture_id,province_id,kingdom_id,page_kind,url,raw_text,raw_payload,parsed_payload,content_hash,captured_at')
...
function extractState(flow, capture) {
  ...
  return compact({
    ...
    content_hash: capture?.content_hash || null,
...
async function quarantine(sb, flow, job, stewardResult) {
  ...
  await sb.from('spartan_data_quarantine').insert({
    capture_id: flow.capture_id,
    source: flow.source,
    ...
    province_id: flow.province_id,
    kingdom_id: flow.kingdom_id,
    steward: 'spartan-stewards',
    severity: stewardResult.decision === 'fail' ? 'critical' : 'high',
    reason: stewardResult.results.map(item => `${item.steward}: ${item.reason}`).join('; '),
    field_name: null,
    observed_value: stewardResult.results,
    raw_payload: capture?.raw_payload || {},
    parsed_payload: capture?.parsed_payload || {},
    validation_details: { confidence: stewardResult.confidence, results: stewardResult.results },
    fingerprint: crypto.createHash('sha256').update(`${flow.flow_id}:${flow.capture_id}`).digest('hex'),
    status: 'quarantined',
  });
...
async function processOne(sb, job) {
  ...
  const { error: projectionError } = await sb.from('spartan_state_projection').upsert({
  ...
  if (integrity.results.some(item => item.steward === 'integrity' && (item.decision === 'fail' || item.decision === 'quarantine'))) {
    throw new Error(`Integrity Steward failed: ${integrity.results.find(item => item.steward === 'integrity')?.reason || 'unknown'}`);
  }
