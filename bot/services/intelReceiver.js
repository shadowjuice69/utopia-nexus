        ...
        if (isCSV) {
          ...
          if (csvLines.length > 1) {
            ...
            for (let ci = 1; ci < csvLines.length; ci++) {
              ...
              if (row.defhome || row.def_home) upsertData.defense = String(parseNum(row.defhome || row.def_home) || row.defhome || row.def_home);
              ...
              if (row["pop_"]) upsertData.be = row["pop_"];
              ...
              if (row.badspells || row["badspells"]) upsertData.bad_spells = row.badspells || row["badspells"];
              if (row.nwpa) upsertData.nwpa = row.nwpa;
              if (row.drafttarget || row.draft_target) upsertData.draft_target = row.drafttarget || row.draft_target;
              if (row.draftrate || row.draft_rate) upsertData.draft_rate = row.draftrate || row.draft_rate;
              ...
              if (row.stlth || row.stealth) upsertData.stealth = row.stlth || row.stealth;
