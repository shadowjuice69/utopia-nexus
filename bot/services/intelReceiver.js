      ...
      try {
        ...
        if (isCSV) {
          ...
          if (csvLines.length > 1) {
            ...
            for (let ci = 1; ci < csvLines.length; ci++) {
              ...
              if (row.off) upsertData.off = String(parseNum(row.off) || row.off);
              if (row.def) upsertData.def = String(parseNum(row.def) || row.def);
              if (row.defhome) upsertData.def_home = String(parseNum(row.defhome) || row.defhome);
              if (row.be) upsertData.be = String(row.be).replace('%','').trim();
              if (row.rtpa) upsertData.r_tpa = parseFloat(row.rtpa) || null;
              if (row.otpa) upsertData.o_tpa = parseFloat(row.otpa) || null;
        ...
        if (isRaw) {
          ...
          if (isTabular) {
            ...
            if (headerIdx === -1) {
              ...
              for (let i = headerIdx + 1; i < lines.length; i++) {
                ...
                const off = get(row, "off"); if (off) upsertData.off = String(parseNum(off) || off);
                const def = get(row, "def"); if (def) upsertData.def = String(parseNum(def) || def);
                const defhome = get(row, "defhome"); if (defhome) upsertData.def_home = String(parseNum(defhome) || defhome);
                const peons = get(row, "peons"); if (peons) upsertData.peasants = parseNum(peons);
                const rtpa = get(row, "rtpa"); if (rtpa) upsertData.r_tpa = parseFloat(rtpa) || null;
                const otpa = get(row, "otpa"); if (otpa) upsertData.o_tpa = parseFloat(otpa) || null;
          ...
          for (const row of siteData.rows) {
            ...
            if (row.off) upsertData.off = parseInt((row.off||"0").replace(/[^0-9]/g,"")) || null;
            if (row.def) upsertData.def = parseInt((row.def||"0").replace(/[^0-9]/g,"")) || null;
            if (row.be) upsertData.be = String(parseInt((row.be||"0").replace(/[^0-9]/g,"")) || "");
