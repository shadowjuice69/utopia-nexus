...
const { showBuildPicker } = require("../investInteractionHandler");
...
module.exports = async function investHandler(interaction) {
  try {
    const buildName = interaction.options.getString("build") || "";
    const categoryBooks = {
    ...
    if (!buildName.trim()) return showBuildPicker(interaction, "", 0, categoryBooks);
    ...
    return interaction.reply({ embeds: [renderOutcome(outcome)], ephemeral: true });
  } catch (err) {
    console.error("[INVEST HANDLER ERROR]", err);
    if (!interaction.replied) await interaction.reply({ content: "❌ Investment calculation failed.", ephemeral: true });
  }
};
