...
function parseSpell(text, self = false) {
  const t = clean(text);
  const head = t.match(/^(?:#?\d+\s*-\s*[^:]+:\s*)?(?:Your wizards gather\s+([\d,]+)\s+runes and begin casting,\s*)?(?:—\s*)?([^:]+):\s+Your wizards gather/i);
  const runes = n(t.match(/gather\s+([\d,]+)\s+runes/i)?.[1]);
  const spell = head ? head[2].trim() : null;
  if (!spell || runes == null) return null;
...
function parseDragon(text) {
  ...
  return {
    ...
    strength: incoming ? n(incoming[2]) : null,
    raw: t
  };
}
...
function parseRitual(text) {
  ...
  return {
    type: 'ritual',
    casterProvince: who?.[2]?.trim() || null,
    casterKingdom: null,
...
function parseAid(text) {
  const t = clean(text);
  if (!/We have sent\s+[\d,]+\s+/.test(t)) return null;
  const m = t.match(/^#?\d+\s*-\s*(.+?):\s*We have sent\s+([\d,]+)\s+(.+?)\s+to\s+(.+?)\s+\((\d+:\d+)\)\./i);
  if (!m) return null;
  const sender = m[1].trim();
  const amount = n(m[2]);
  const resource = m[3].trim().toLowerCase();
  const surplus = n(t.match(/added\s+([\d,]+)\s+gold coins to our aid surplus/i)?.[1]);
  return { type: 'aid', senderProvince: sender, senderKingdom: null, targetProvince: m[4].trim(), targetKingdom: m[5], resourceType: resource, amount, surplusGold: surplus, raw: t };
}
...
function parseAttack(text) {
  ...
  if (!Object.values(loot).some(v => v != null)) Object.assign(loot, {});
  ...
  return { type:'attack', attackType:/recaptured/i.test(t)?'recapture':/ambush/i.test(t)?'ambush':/massacred/i.test(t)?'massacre':/looted/i.test(t)?'loot':'offensive', attackerProvince:battle[3].trim(), attackerKingdom:battle[2], targetProvince:target?.[1]?.trim() || null, targetKingdom:target?.[2] || null, acresCaptured:/recaptured/i.test(t)?null:acres, acresRecaptured:/recaptured/i.test(t)?acres:null, acresDestroyed:/razed|destroyed/i.test(t)?acres:null, offenseSent:off || sent, enemyDefense:defense, kills:killed, prisoners:imprisoned, losses, loot, returnDays:Number.isFinite(returnDays)?returnDays:null, raw:t };
}
...
function parseChannelMessage({ channelType, id, content, timestamp }) {
  ...
  for (const chunk of String(content || '').split(/\n(?=⚔|🐉|#?\d+\s*-)/)) {
    const event = parser(chunk);
    if (event) out.push({ ...event, messageId:id, timestamp });
  }
  return out;
}
